...
session_start();
  ...
  header('location:login.php');
    ...
    $hid = $_SESSION['hid'];
    $sql = "select plasmarequest.*, receivers.* from plasmarequest, receivers where hid='$hid' && plasmarequest.rid=receivers.id";
...
<body>
	...
	<table class="table table-responsive table-striped rounded mb-5">
		<tr><th colspan="9" class="title">Plasma requests</th></tr>
		<tr>
			<th>#</th>
			<th>Name</th>
			<th>Email</th>
		...
		<tr>
			...
			<td><?php if($row['status'] == 'Accepted'){ ?> <a href="" class="btn btn-success disabled">Accepted</a> <?php }
				...
				<a href="file/paccept.php?reqid=<?php echo $row['reqid'];?>" class="btn btn-success">Accept</a>
			<?php } ?>
			</td>
			<td><?php if($row['status'] == 'Rejected'){ ?> <a href="" class="btn btn-danger disabled">Rejected</a> <?php }
			else{ ?>
				<a href="file/preject.php?reqid=<?php echo $row['reqid'];?>" class="btn btn-danger">Reject</a>
			<?php } ?>
			</td>
